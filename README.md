...
Open `index.html` in a browser to cycle random flops and see the current nut hole cards plus the strongest ranked hand tiers. Click any board card to replace it from the deck, or click the Turn/River slots to add later streets; cards already on the board are disabled in the picker. Existing Turn/River cards can also be removed from the picker, and removing the Turn clears the River too. Add your own two-card hand to see its estimated win chance and current tier versus the selected opponent count. The evaluator ranks every legal two-card hole combo against the current board, so a flop checks 1,176 possible hands, a turn checks 1,128, and a river checks 1,081. Each visible tier shows its percentage share of those possible hole-card combos and an estimated showdown win percentage, including split-pot equity, against the selected number of random opponents.
