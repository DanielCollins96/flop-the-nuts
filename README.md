...
Open `index.html` in a browser to cycle random flops and see the current nut hole cards plus the strongest ranked hand tiers. The evaluator ranks every legal two-card hole combo against the three-card flop, so each deal checks 1,176 possible hands. Each visible tier shows its percentage share of those possible hole-card combos and an estimated showdown win percentage, including split-pot equity, against the selected number of random opponents.
